...
async function main() {
  ...
  try {
    ...
    if (!cashoutNames.includes("ibanBic") || !cashoutNames.includes("walletChain")) {
      throw new Error(
    ...
    console.log(
      "Schema SQLite ok: LedgerEntry.chfCents, CashoutRequest.ibanBic/walletChain e GatewayTransmission presenti.",
    );
  } finally {
