...
7. **Conversione tesoreria** — Massimo converte crediti ancora in casa in **euro, dollari, franchi o crypto** (BTC, ETH, USDT, USDC, BNB). Lo **spread %** per €/$/CHF/USDC (Forgia) resta in tesoreria a libro; solo il netto entra in cassa. Euro, dollari e franchi partono verso gli IBAN casa (UniCredit / Wise) nello stesso passo. Le crypto EVM tentano mint a gas zero su Zecca Gasless verso il wallet indicato. **USDC convertito è solo libro** — deposita USDC vero sul SCA. **Arrivo crypto EVM = tx_hash su /catena.** **Arrivo banca = TRN.** Senza `ZECCA_SEPA_GATEWAY_*` o Wise Platform i destinatari IBAN non ricevono.
...
**USDC in uscita (automatico, Circle SCA + Gas Station, Base mainnet):** il cliente incolla un `0x…` — MetaMask, Trust Wallet, o il **deposito USDC sulla rete Base** di Kraken/MEXC (non Ethereum, non un altro token). Tasso: 1 credito = `usdCentsPerCredit` (**1 USDC = 1 USD** di libro). Netto inviato = richiesta − (fissa + % impostate in Forgia). La commissione **non viene trasferita**: resta nel SCA. Cap: netto ≤ min(cassa libro, saldo Circle − commissione trattenuta). Alla conferma Zecca chiama lato server `POST /v1/w3s/developer/transactions/transfer` con `feeLevel: MEDIUM` e `amounts` = **solo il netto**. Su un wallet **SCA**, se in Console esiste una **policy Gas Station di default su BASE**, Circle sponsorizza il **gas del sender** (negozio) addebitandolo sul conto Circle: **nessun flag extra** nella request, il destinatario **non firma** e **non paga il gas**. Il copy in app lo dice così. Il conio di crediti **non** crea USDC: gli euro veri restano Stripe/bonifici cliente; senza saldo USDC sul wallet negozio l’invio non parte.
